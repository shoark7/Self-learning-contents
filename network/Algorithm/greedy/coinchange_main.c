...
int main(void)
{
	...
	if(item_count > 0 && change_total > 0){
		items = malloc(sizeof(struct coin_item) * item_count);

		if(NULL != items){
			input_items(items, item_count);
			remained = get_change(change_total, items, item_count);
			print_result(items, item_count, remained);
