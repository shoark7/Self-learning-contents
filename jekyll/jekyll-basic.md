...
## 0. Jekyll 설치
Jekyll은 심플한 블로그 지향적 정적 사이트 생성 프로그램이다.
...
* 참고: `jekyll build` 명령어도 있다. 이 명령어는 `jekyll serve`와 달리, **로컬 서버로 호스팅하지 않고**  
\_site 폴더에 사이트만 만든다.  `--watch` 옵션으로 변경사항이 생길 때마다 자동으로 다시 생성도 가능.  
...
> 현재 경로에 Jekyll 프로젝트를 구성하고 싶다면 `jekyll new .`
...
Jekyll은 프로그램이고, 프로그램으로서의 자신만의 특성이 존재한다.  
이는 Jekyll을 사용하기 위해서는 '공부'해야 하며 이를 꽉 잡고 가면 이후 사용에 큰 도움이 되리라 믿는다.

### 4.1. 머리말
Jekyll은 `YAML 머리말 블록`을 가진 파일을 특별한 파일로 인식해 표현한다.  
머리말은 반드시 올바른 YAML 형식으로 처리되어야 하며, `대시문자 3개`로 파일의 **맨 처음**에 위치해야 한다.
...
이 대시문자 사이에 사전에 정의된 변수를 사용할 수 있고, 나만의 고유변수를 정의할 수도 있다.  
이 변수들은 추후 해당 페이지 또는 이 페이지에 연결된 레이아웃이나 include 파일에서 접근할 수 있다.  

사전 정의된 변수들은 중요하며 따라서 몇 개는 살펴보도록 하자.

* layout: 사용할 레이아웃 파일을 지정하며, 반드시 `_layout` 디렉토리 안에 있어야 한다.
* permalink: 생성된 블로그 포스트 URL을 커스텀하게 지정한다. 
* published: 사이트가 생성되었을 때 특정 포스트가 나타나지 않게 하려면 이 값을 `false`로 한다.
* category, categories: 포스트를 특정 폴더에 넣지 않고, 속할 카테고리를 하나 이상 지정할 수 있다.  
			두 개 이상은 YAML리스트나 쉼표로 구분된 문자열을 사용한다.
* tags: categories와 유사하게 하나 이상의 태그를 포스트에 추가할 수 있다. 사용법은 같다.

* date: 각 포스트마다 이를 입력해주면 좋다. 포스트 파일 제목의 날짜보다 우선한다.
* page: **하나의 포스트에서 포스트 자신을 참조한다.**
...
### 4.2. Liquid 템플릿 문법
장고 등 웹 프레임워크를 공부해본 사람이면 굉장히 익숙하지만, 안 해본 사람들은 처음엔 많이 헷갈릴 수도 있다.  
HTML 등 템플릿을 pre-processing 해주는 문법인데, 다시 말해 최종 HTML에 어떤 내용이 담길지 알 수 없고,  
그걸 다 할 이유도 없을 때 쓴다. 가령 내 블로그 포스트가 총 10개 이고, HTML에 이들의 제목과 글 내용을 삽입한다고 하자.  
그러면 내가 div 등을 총 10개를 입력하는 게 맞나? 이건 굉장히 귀찮은 일이다. 100개, 1000개 넘어가면 할 수 없다.  
...
익숙한 HTML 파일인데 중간에 `{% ... %}`, `{{ ... }}`은 처음 볼 수도 있다.  
이는 **이는 템플릿 엔진이 해석해서 필요한 내용을 HTML에 삽입하라는 템플릿 언어이다.**  

첫 번째 `{% .. %}`에서 'site.posts'는 \_posts의 모든 포스트를 의미하는데, 각 포스트마다  
for문을 돌리라는 의미이다. 그러면 `{% for .. %}`과 `{% endfor %}` 사이의 블록이 포스트 개수만큼 실행된다.  

두 번째 `{{ .. }}`는 변수값이 들어간다. 내가 정의했든, 정해져 있든 변수가 들어간다.  
`{{ post.url }}`에서는 post의 url값이, `{{ post.title }}`에서는 post의 title 값이 최종적으로 삽입된다.  

자세한 설명은 [여기](http://jekyllrb-ko.github.io/docs/templates/)에서 볼 수 있다.  
여기서 설명하기는..
